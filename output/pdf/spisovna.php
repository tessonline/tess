...
if ($_POST['box'] == 1) {
  ...
  if (count($IDcka) == 0) $IDcka = array(-1);
  
  $_POST['sql'] = 'select * from posta where id in (' . implode(',', $IDcka) .')';
}
...
//      $this->Row(array('jednací číslo','nazev','Rok vzniku','Sk.znak a lhůta','Listů','Místo uložení','Pozn.o trvalém vyřazení'));
      $this->Row(array('Box','Jednací číslo/spis','Odbor','Název','Datum vzniku','Sk.znak a lhůta','Listů','Digitální','Přílohy'));
...
if (is_array($_POST['SELECT_IDdenik'])) {
  if (count($_POST['SELECT_IDdenik'])>0)
    $_POST['sql'] = str_replace(' ORDER BY ', 'and p.id in (' . implode(',',$_POST['SELECT_IDdenik']). ') ORDER BY ', $_POST['sql']);
}
...
$pdf->SetWidths(array(20,35,35,10,25,20,10,10,20));
...
$pdf->Row(array('Box','Jednací číslo/spis','Název','Odbor','Datum vzniku','Sk.znak a lhůta','Listů','Digitální','Přílohy'));
$pdf->SetFont('Arial','',7);
...
while($q->next_record()) {
  ...
  $pocetl=$q->Record['POCET_LISTU'];
   list($znak,$pismeno,$lhuta)=Skartace($q->Record["SKARTACE"]);    
...
//  $sql="select id,typ_dokumentu,skartace,pocet_listu,pocet_priloh,druh_priloh from posta where (".$where_spis.") and superodbor in (".VratSuperOdbor().") and stornovano is null order by id asc";
  $sql="select id,typ_dokumentu,skartace,pocet_listu,pocet_priloh,druh_priloh from posta where (".$where_spis.") and stornovano is null order by id asc";
  $a->query ($sql);
  $CISLO_JEDNACI_SPIS = array();
  $CISLO_JEDNACI_SPIS[] = $CISLO_JEDNACI;
  while($a->next_record()) {
    ...
    $doc2 = LoadSingleton('EedDokument', $a->Record['ID']);
    if (!in_array($doc2->cislo_jednaci,$CISLO_JEDNACI_SPIS)) $CISLO_JEDNACI_SPIS[] = $doc2->cislo_jednaci;
...
//    $doc = LoadSingleton('EedDokument', $id);
    ...
    if (count($CISLO_JEDNACI_SPIS)>1) {
      $CISLO_JEDNACI=''. implode('
', $CISLO_JEDNACI_SPIS).'
-----------';
    }
...
VNITŘNÍ POŠTA';
    ...
    $boxy = LoadClass('EedArchivacniBox', 0);
    $pole = $boxy->GetBoxProPostaID($id);
    $box_out = '';
    if (count($pole)>0) {
      while (list($key,$val) = each($pole))
      $box_out .= '
'.$val['EV_CISLO'] . ' - ' . $val['NAZEV'];
    }
    $datum=$q->Record["DATUM_PODATELNA_PRIJETI"];
    $znak=$znak." - ".$pismeno."/".$lhuta;
    $cj1=$q->Record["CISLO_SPISU1"] ? $q->Record["CISLO_SPISU1"] : 0;
    $cj2=$q->Record["CISLO_SPISU2"] ? $q->Record["CISLO_SPISU2"] : 0;
...
//    $pdf->Row(array($CISLO_JEDNACI,$VEC,$datum,$pismeno,$pocetl,'',''));
    $pdf->Row(array($box_out,$CISLO_JEDNACI,$VEC,$odbor,$datum,$znak,$pocet_listu_celkem,$digitalni,$prilohy,''));
