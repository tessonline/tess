...
If ($GLOBALS[SUPERODBOR]) $where .= " AND superodbor=  ".$GLOBALS[SUPERODBOR]."";
...
while (($a<$pocet)) {
  If ($GLOBALS["ID"] or $GLOBALS["CELE"] or (!$GLOBALS["EV_CISLO_DO"]))  $q->Next_Record();
  If ($radek>$radku) {
    $pdf->AddPage();
    $radek = 1;  
  }
...
//  $b->Next_Record();
  ...
  if (strlen($GLOBALS["CONFIG_POSTA"]["HLAVNI"]["ADRESA_RAZITKO"])>3)
    list($nazev_uradu,$nazev_uradu2) = explode('|',$GLOBALS["CONFIG_POSTA"]["HLAVNI"]["ADRESA_RAZITKO"]);
...
//  $pdf->Code39_otoc(111+$pozicex,43+$pozicey,$podaci_cislo,1,8);
  ...
  //pro tisk predpripravenych stitku
  If (!$GLOBALS["CELE"]&&!$GLOBALS["ID"]&&$GLOBALS["EV_CISLO_DO"]) {
    $podaci_cislo=($a+$GLOBALS["EV_CISLO_OD"])."/".Date('Y');
    if ($GLOBALS['POMLCKA']) $podaci_cislo=($a+$GLOBALS["EV_CISLO_OD"])."-".Date('Y');
  }
