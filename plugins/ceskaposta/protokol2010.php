...
//Die($sql);
$q->query($sql);
$q->Next_Record(); $obyc1=$q->Record['OBYC1'];$obyc2=$q->Record['OBYC2'];
$protokol=$q->Record['PROTOKOL'];
$GLOBALS['datum_od']=$q->Record['DATUM_OD'];
$GLOBALS['datum_do']=$q->Record['DATUM_DO'];
$protokol = $q->Record['PROTOKOL'].'/'.$q->Record['ROK'];
...
class PDF extends FPDF
{
  ...
  function Header()
  {
    global $protokol;
     $this->SetFont('Arial','B',11);
      //Move to the right
     $this->SetXY(1,5);
     $this->Cell(210,4,"POŠTOVNÍ PODACÍ ARCH - " . $protokol,0,0,C);
     $this->Ln(5);
  } 
  ...
  //    $this->Cell(0,3,'vytištěno v programu Evidence dokumentů, (c) T-MAPY spol. s r.o., www.tmapy.cz',0,0,'L');
      $this->Cell(0,3,'Stránka '.$this->PageNo().'/{nb}, tisk dne '.$datum,0,0,'L');
      //$this->Cell(0,3,'vytištěno v programu Evidence dokumentů, (c) T-MAPY spol. s r.o., www.tmapy.cz',0,0,'L');
      $this->Cell(0,3,'Potvrzení pošty (otisk razítka APOST)                ',0,0,'R');
  }
}
...
include('pdf_inc/tabulka2010.inc');
$pdf->SetXY(255.3,14.7);
$pdf->SetFont('Arial','',14);
$pdf->Cell(23.6,10.6,$protokol,1,1,C);
...
$sql="select * from posta where id in (select pisemnost_id from posta_cis_ceskaposta_id where protokol_id=".$GLOBALS[ID].") and doporucene<>'1' ORDER BY xertec_id,doporucene,cislo_jednaci2,cislo_jednaci1 asc";

if ($GLOBALS['CONFIG_POSTA_CESKAPOSTA']['TRIDIT_DLE_DATUM_PREDANI'])
  $sql="select * from posta where id in (select pisemnost_id from posta_cis_ceskaposta_id where protokol_id=".$GLOBALS[ID].") and doporucene<>'1' ORDER BY doporucene,datum_predani_ven,xertec_id,cislo_jednaci2,cislo_jednaci1 asc";
...
while ($q->Next_Record())
{
	If (($a>10)or($puvodni<>$q->Record["DOPORUCENE"] && $puvodni=='Z')or($puvodni<>$q->Record["DOPORUCENE"] && $puvodni=='C')or($puvodni<>$q->Record["DOPORUCENE"] && $q->Record["DOPORUCENE"]=='C' && $stranka>1))
//	If ($a>15)
	{
    $pdf->AddPage('P');
		$pdf->Ln();
...
//  $pdf->SetXY(81,$b+2);
//   $pdf->Cell(52,3,$q->Record["ODESL_PRIJMENI"].' '.$q->Record["ODESL_JMENO"],0,0);
//   $pdf->SetXY(81,$b+6);
...
// //  $pdf->Cell(52,3,$ulice,0,0,'L');
//   $pdf->SetXY(81,$b+9);
//   $pdf->MultiCell(52,3,$ulice,0,0,'L');
//
//   $pdf->SetXY(81,$b+10);
//   $pdf->Cell(52,3,substr($q->Record["ODESL_PSC"],0,3).' '.substr($q->Record["ODESL_PSC"],3,2).'  '.$q->Record["ODESL_POSTA"],0,0);
//   $pdf->SetXY(119,$b+1);
//
  ...
  $adresa = '';
  if ($q->Record["ODESL_OTOCIT"]>0)
  {

    $odd = '';
    If (trim($q->Record["ODESL_ODD"])<>'') $odd = $q->Record["ODESL_ODD"];
    If (trim($q->Record["ODESL_ODD"])<>'' && $q->Record["ODESL_OSOBA"]) $odd .= ','.$q->Record["ODESL_OSOBA"];
    If (!$q->Record["ODESL_ODD"] && $q->Record["ODESL_OSOBA"]) $odd = $q->Record["ODESL_OSOBA"];
    if (strlen($odd)>2) $adresa .= $odd.'
' . $q->Record["ODESL_PRIJMENI"].' '.$q->Record["ODESL_JMENO"];
    else
      $adresa .= $q->Record["ODESL_PRIJMENI"].' '.$q->Record["ODESL_JMENO"];
  }
  else {
    $odd = '';
    If (trim($q->Record["ODESL_ODD"])<>'') $odd = $q->Record["ODESL_ODD"];
    ...
    If (!$q->Record["ODESL_ODD"] && $q->Record["ODESL_OSOBA"]) $odd = $q->Record["ODESL_OSOBA"];
  //  $pdf->Cell(52,3,$ulice,0,0,'L');
    if (strlen($odd)>2) $adresa .= $q->Record["ODESL_PRIJMENI"].' '.$q->Record["ODESL_JMENO"].'
' . $odd;
    else
      $adresa .= $q->Record["ODESL_PRIJMENI"].' '.$q->Record["ODESL_JMENO"];
  ...
  $adresa .= '
' . $ulice.'
'.substr($q->Record["ODESL_PSC"],0,3).' '.substr($q->Record["ODESL_PSC"],3,2).'  '.$q->Record["ODESL_POSTA"];

 $pdf->MultiCell(52,3,$adresa,0,'L');
...
/*  
  ...
  */
  ...
  If ($q->Record["DOPORUCENE"]=='Z') {$text='';$cislood--;}
  ...
  $pdf->Cell(30,4,$text,0,0,C);
  $pdf->SetXY(174,$b+4);
  $pdf->SetFont('Arial','',6);
  $pdf->Cell(30,4,$cj,0,0,C);

  if ($GLOBALS['CONFIG_POSTA_CESKAPOSTA']['V_PROTOKOLU_SPIS_ZKRATKA']) {
    $pdf->SetXY(174,$b+8);
    $pdf->Cell(30,4,$uzel,0,0,C);
  }
...
$pdf->SetFont('Arial','B',10);
  $b=(($a-1)*21)+42;
  $pdf->SetXY(81,$b+5);
  $pdf->MultiCell(52,3,$obyc1.' obyčejných zásilek','0','0');
}
if ($obyc2>0)
{
$pdf->SetFont('Arial','B',10);
  $b=($a*21)+42;
  $pdf->SetXY(81,$b+5);
  $pdf->MultiCell(52,3,$obyc2.' zásilek ZR','0','0');
}
/*
