...
//    $sql="select * from posta where".$where." and doporucene='".$key."' and vlastnich_rukou='".$key1."' order by ID";
    $sql='select * from posta where id in (select pisemnost_id from posta_cis_dorucovatel_id where protokol_id='.$GLOBALS[ID].') and doporucene=\''.$key.'\'  ORDER BY odbor,doporucene,datum_podatelna_prijeti';
//    echo $sql;
    $q->query ($sql);
    $pocet=$q->Num_Rows();
    ...
    while($q->next_record())
    {
      ...
      $cj_obj = LoadClass('EedCj', $q->Record['ID']);
