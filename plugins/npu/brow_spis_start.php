...
*/
  ...
  //    if ($lze_editovat)
    ...
    if ($spis_uzavren && $mamPravaNaSpis && ($cj_info['STATUS']>0 || $cj_info['STATUS']==-4) && $je_spisovak ) echo "<a  class=\"btn btn-loading\" href=\"".GetAgendaPath("SKARTACNI_KNIHA",false,true)."/brow.php?ID=".$spis_id."&PREDANI=1\">Předat do spisovny</a>";
...
//        $sql = 'select * from cis_posta_spisy where spis_id='.$GLOBALS['spis_id'].' and dalsi_spis_id=0';
      ...
      if ($cj_info['JE_SPIS'] && !$cj_info['JE_TYPOVY_SPIS'] && $mamPravaNaSpis) echo "<a class=\"btn btn-loading\" href=\"#\" onclick=\"javascript:NewWindowAgendaSpis('".GetAgendaPath('POSTA',false,true)."/guide.php?G_NAME=posta_dalsispisy&SPIS_ID=".$spis_id."');\">Přidat do spisu číslo jednací</a></p>";
      if ($cj_info['JE_SPIS'] && $cj_info['JE_TYPOVY_SPIS'] && $mamPravaNaSpis) echo "<a class=\"btn btn-loading\" href=\"#\" onclick=\"javascript:NewWindowAgendaSpis('".GetAgendaPath('POSTA',false,true)."/guide.php?G_NAME=posta_dalsispisy&SPIS_ID=".$spis_id."');\">Přidat do dílu číslo jednací</a></p>";
...
//  If ((HasRole('spravce')||HasRole('vedouci-odbor')||HasRole('editace_uzel')||HasRole('editace_admin')||HasRole('vedouci-odd')) && $mamPravaNaSpis && !$spis_uzavren && !$GLOBALS["FROM_EVIDENCE"] && !$cj_info['JE_TYPOVY_SPIS'])  {
  If (HasRole('zmena-spisoveho-uzlu') && (HasRole('editace_zpracovatel')||HasRole('editace_uzel')||HasRole('editace_admin'))&& $mamPravaNaSpis && !$spis_uzavren && !$GLOBALS["FROM_EVIDENCE"] && !$cj_info['JE_TYPOVY_SPIS'])  {
...
//    "sql" => "select * from posta WHERE ".$where_cj.  "  order by id desc",
  ...
  if ($GLOBALS['SHOW_CELY_SPIS']  == 1)
    ...
    $GLOBALS['ID'] = $cj_obj->GetDocsInCj($spis_id);
