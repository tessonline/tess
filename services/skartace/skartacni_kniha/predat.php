...
while ($q->Next_Record()) {
  ...
  $sqlb = 'update posta set spisovna_id=' . $spisovna_id . ' where id=' . $dokument_id;
  ...
  $text = 'Spis/čj. <b>' . $cj. '</b> byl předán do jiné spisovny: <b>' . $spisovna_text . '</b>';
  EedTransakce::ZapisLog($dokument_id, $text, 'SPIS', $id_user);

}

echo '<h1>Dokumenty byly předány.</h1>';
echo "
