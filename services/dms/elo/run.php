...
$client = new tw_soapclient($GLOBALS["PROPERTIES"]["ELO_DMS_IP_PORT"], true);
  if ($GLOBALS["PROPERTIES"]["ELO_DMS_ENDPOINT"])
    $client->setEndpoint($GLOBALS["PROPERTIES"]["ELO_DMS_ENDPOINT"]);
$err = $client->getError();
if ($err) {
    echo "<input type='button' class='button btn' value='   Zavřít okno   ' onclick='window.close();'><BR/>";
    Die('ELO systém není dostupný');
}
//Connect do ELO
$result = $client->call('Login', array(
  new soapval('','Login',array(
    'lstrUSRName'=>$PROPERTIES["ELO_DMS_LOGIN"],
    'lstrUSRPsw'=>$PROPERTIES["ELO_DMS_PASSWD"],
    'lstrArchivName'=>$PROPERTIES["ELO_DMS_ARCHIV"],
  ...
  if ($pocet==0)
  {
    $result = $client->call('CreateBCDoc', array(
      new soapval('','CreateBCDoc',array(
        'lnExternalID'=>$podaci_cislo,
        'lstrTicketId'=>$session, 
      ),false,'http://tempuri.org/ELO/ELOHTTP')
    ));
    //  print_r($result);
    $id=$result['CreateBCDocResult'];
    If ($id=="-2") echo "Zaznam ".$podaci_cislo." je jiz vytvoren - vraceno id ".$id."<br/>";
    elseIf ($id<1) echo "Zaznam ".$podaci_cislo." nebyl vytvořen - nelze oskenovat! - vraceno id ".$id."<br/>";
    else
    {
    ...
    If ($id=="-2") echo "Zaznam ".$podaci_cislo." je jiz vytvoren <br/>";
...
echo "<input type='button' class='button btn' value='   Zavřít okno   ' onclick='window.close();'>";
