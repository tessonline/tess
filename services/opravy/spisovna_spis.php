...
$sqla="select s.id, p.id as ID_doc,s.dokument_id,s.cislo_spisu1,s.cislo_spisu2,s.last_date
  ...
  and s.dokument_id in (select spis_id from cis_posta_spisy);
";
$q->query($sqla);
...
while ($q->Next_Record())
{
  $id_doc = $q->Record['DOKUMENT_ID'];
  $id = $q->Record['ID'];
  $sqla = 'select * from posta_view_spisy where id=' . $id_doc;
  $b->query($sqla);
  $b->Next_Record();

  $cj1 = $b->Record['CISLO_SPISU1'];
  $cj2 = $b->Record['CISLO_SPISU2'];
  $so = $b->Record['SUPERODBOR'];
  $vec = $b->Record['VEC'];
  $sqlb="UPDATE posta_spisovna SET vec='$vec',cislo_spisu1=$cj1,cislo_spisu2=$cj2,superodbor=$so,last_date='$LAST_DATE',last_time='$LAST_TIME',last_user_id=$LAST_USER_ID WHERE id=".$id;
  echo $sqlb.'<br/>';
  $b->query($sqlb);
}
