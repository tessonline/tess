...
// povinne parametry
    ...
    $q=new DB_POSTA;
    $sql = "SELECT c.*,c.id as idcko,s.* FROM cis_posta_odbory c LEFT join security_group s ON c.id_odbor = s.id ";
//    if ($a_odbor) $sql.=" WHERE id = '".$a_odbor."'";
/*    if ($a_id) $sql.=" AND id = '".$a_prac."'";
    if ($a_idod) $sql.=" AND id > '".$a_idod."'";*/
    $sql.=" order by id_odbor;";
    $q->query($sql);
...
// hlavicka XML
    ...
    while($q->next_record())
    {
      ...
      echo "    <b_id>".$q->Record["IDCKO"]."</b_id>\n";
      echo "    <b_odbor>".$q->Record["NAME"]."</b_odbor>\n";
      echo "    <b_zkratka>".$q->Record["ZKRATKA"]."</b_zkratka>\n";
