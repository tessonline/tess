...
/*
$Id: nusoap.php,v 1.20 2009/12/01 12:49:19 tohu Exp $
...
* @version  $Id: nusoap.php,v 1.20 2009/12/01 12:49:19 tohu Exp $
* @access   public
*/
class nusoap_base {
	/**
                ...
                if($valueType=='arraySimple' || ereg('^ArrayOf',$type)){
					...
					foreach($val as $k => $v){
						...
						if ($type == 'Map' && $type_ns == 'http://xml.apache.org/xml-soap') {
							...
							$xml .= '</item>';
            } elseif ($type == 'VectorMap') {
              foreach($v as $vk => $vv) $xml .= $this->serialize_val($vv,$vk,false,false,false,false,$use);
						} else {
  ...
  if ($this->signature['active']) {
  	...
  	$this->appendDebug($this->varDump($this->signature));
    switch ($this->signature['version']) {
    //dalsi verse nutno dopsat
    case 'wss-v1.1':
    default:
      $headers .= '<wsse:Security xmlns:wsse="http://docs.oasis-open.org/wss/oasis-wss-wssecurity-secext-1.1.xsd">';
      $headers .= '<ds:Signature xmlns:ds="http://www.w3.org/2000/09/xmldsig#">';
      $headers .= '<ds:SignedInfo>';
      $headers .= '<ds:CanonicalizationMethod Algorithm="http://www.w3.org/TR/2001/REC-xml-c14n-20010315"/>';
      $headers .= '<ds:SignatureMethod Algorithm="http://www.w3.org/2001/04/xmldsig-more#rsa-sha512"/>';
      $headers .= '<ds:Reference URI="#MsgBody">';
      $headers .= '<ds:Transforms>';
      $headers .= '<ds:Transform Algorithm="http://www.w3.org/TR/2001/REC-xml-c14n-20010315#WithComments"/>';
      $headers .= '</ds:Transforms>';
      //Digest method and Value
      switch ($this->signature['digestmethod']) {
        case 'sha1':
          $headers .= '<ds:DigestMethod Algorithm="http://www.w3.org/2000/09/xmldsig#sha1"></ds:DigestMethod>';
          $headers .= '<ds:DigestValue>'.base64_encode(sha1($body)).'</ds:DigestValue>';
        break;
        default:
          die('NUSOAP: unsupported digest method');
        break;
      }
      $headers .= '</ds:Reference>';
      $headers .= '</ds:SignedInfo>';
      //Signature value
      $this->debug("signature key: ".$this->signature['key']);
      // získat ze souboru soukromý klíè a pøipravit ho
      $fp = fopen($this->signature['key'], "r");
      $keyContent = fread($fp, 8192);
      fclose($fp);
      //nacteme si privatni klic z PEM klice
      $pkeyid = openssl_get_privatekey($keyContent);  
      // vytvoøit podpis
      openssl_sign($body, $signatureData, $pkeyid);
      // uvolnit klíè z pamìti
      openssl_free_key($pkeyid); 
      //prevedeme do base64
      $signature64=base64_encode($signatureData);
      //ziskame verejny klic z klice
      $keyContentLines = explode("\n", $keyContent);
      $writeKey = false;
      $X509Certificate = '';
      foreach ($keyContentLines as $line) {
        switch ($line) {
          ...
          break;
          default:
            if ($writeKey) $X509Certificate .= ($X509Certificate?"\n":'').$line;
          break;
        }
      }
      $DERCertificate = base64_decode(Str_Replace("\n", '', $X509Certificate));
      
      //Decode Modulus and Exponent
      include_once($GLOBALS['TMAPY_LIB'].'/lib/nusoap/lib/ASNValue.class.php');
      $DERASNValue = new ASNValue;
      $DERASNValue->Decode($DERCertificate);
      $DERASNValueItems = $DERASNValue->GetSequence();
      $RSAModulus = $DERASNValueItems[1]->GetIntBuffer();
      $RSAModulus = base64_encode($RSAModulus);
      $RSAExponent = $DERASNValueItems[2]->GetIntBuffer();
      $RSAExponent = base64_encode($RSAExponent);
      
      $headers .= '<ds:SignatureValue>'.$signature64.'</ds:SignatureValue>';
      $headers .= '<ds:KeyInfo>';
      $headers .= '<ds:X509Data>';
      $headers .= '<ds:X509Certificate>'.$X509Certificate.'</ds:X509Certificate>';
      $headers .= '</ds:X509Data>';
      $headers .= '<ds:KeyValue>';
      $headers .= '<ds:RSAKeyValue>';
      $headers .= '<ds:Modulus>'.$RSAModulus.'</ds:Modulus>';
      $headers .= '<ds:Exponent>'.$RSAExponent.'</ds:Exponent>';
      $headers .= '</ds:RSAKeyValue>';
      $headers .= '</ds:KeyValue>';
      $headers .= '</ds:KeyInfo>';
      $headers .= '</ds:Signature>';
    ...
    */
	function getNamespaceFromPrefix($prefix){
		...
		if (is_array($this->schemas)) {
  		foreach ($this->schemas as $val) {
  		  foreach ($val as $sch) {
    		  if (isset($sch->namespaces[$prefix])) {
    		    return $sch->namespaces[$prefix];
          }
        }
      }
    }
    ...
    */
	function getPrefixFromNamespace($ns) {
		...
		if (is_array($this->schemas)) {
  		foreach ($this->schemas as $val) {
  		  foreach ($val as $sch) {
  		    if (is_array($sch->namespaces)) {
            foreach ($sch->namespaces as $p => $n) {
        			if (($ns == $n || $ns == $p) && $p != 'tns') {
        			    $this->usedNamespaces[$p] = $n;
        			    $sch->namespaces[$p] = $n;
        				return $p;
...
class XMLSchema extends nusoap_base  {
		...
		switch($name){
			...
			case 'complexType':
				...
				if(isset($attrs['name'])){
					...
					if(isset($attrs['base']) && ereg(':Array$',$attrs['base'])){
						...
						$this->complexTypes[$this->currentComplexType]['phpType'] = 'array';
					//TOKR 2007-02-02
					} elseif (preg_match('/^ArrayOf/i', $attrs['name'])) {
						$this->xdebug('complexType has ArrayOf prefix (ArcGIS)');
						$this->complexTypes[$this->currentComplexType]['phpType'] = 'array';
					//TOKR END
					} else {
						$this->complexTypes[$this->currentComplexType]['phpType'] = 'struct';
			...
			case 'extension':	// simpleContent or complexContent type extension
				...
				if ($this->currentComplexType) {
					//TOKR added 2007-02-01
					//$this->complexTypes[$this->currentComplexType]['extensionBase'] = $attrs['base'];
					$this->complexTypes[$this->currentComplexType]['extensionBase'] = (StrPos($attrs['base'],':',1)?'':$this->defaultNamespace[$pos].':').$attrs['base'];
					//TOKR end
				}
			break;
			case 'import':
			...
			case 'simpleContent':	// (optional) content for a complexType
				if ($this->currentComplexType) {
					$this->complexTypes[$this->currentComplexType]['phpType'] = 'scalar';
				}
			break;
			case 'simpleType':
    ...
    * @see addElement
    */
	//TOKR 2009-07-31 added entity_type support the same name of element and type
	function getTypeDef($type, $entity_type=false){
	//function getTypeDef($type){
		//$this->debug("in getTypeDef for type $type");
		if(isset($this->complexTypes[$type]) && (!$entity_type || $entity_type == 'complexType')){
			$this->xdebug("in getTypeDef, found complexType $type");
			return $this->complexTypes[$type];
		} elseif(isset($this->simpleTypes[$type]) && (!$entity_type || $entity_type == 'simpleType')){
		...
		} elseif(isset($this->elements[$type]) && (!$entity_type || $entity_type == 'element')){
		...
		} elseif(isset($this->attributes[$type]) && (!$entity_type || $entity_type == 'attribute')){
...
class soapval extends nusoap_base {
	...
	*/
	//TOKR 2009-07-31 added entity_type support the same name of element and type
	function soapval($name='soapval',$type=false,$value=-1,$element_ns=false,$type_ns=false,$attributes=false, $entity_type=false) {
...
//		}
        ...
        for ($i=0; $i<count($digestElements); $i++) {
          $val = $digestElements[$i];
          if (ereg("=$",$val)) {
          $digestRequest[ereg_replace("^ *,? *(.*)=$","\\1",$val)] = $digestElements[++$i];
          }
	...
	*/
	function invoke_method() {
		...
		if (strpos($this->methodname, '..') > 0) {
			...
			$delim = '.';
		} else if (strpos($this->methodname, '__') > 0) {
			$delim = '__';
		} else {
			$delim = '';
		}
		...
		if ($this->wsdl) {
			...
			if ($this->opData['style'] == 'rpc') {
				...
				if ($this->opData['output']['use'] == 'literal') {
					$payload = '<'.($this->responseMethodname ? $this->responseMethodname : $this->methodname.'Response').' xmlns="'.$this->methodURI.'">'.$return_val.'</'.($this->responseMethodname ? $this->responseMethodname : $this->methodname.'Response').">";
				} else {
					$payload = '<ns1:'.($this->responseMethodname ? $this->responseMethodname : $this->methodname.'Response').' xmlns:ns1="'.$this->methodURI.'">'.$return_val.'</ns1:'.($this->responseMethodname ? $this->responseMethodname : $this->methodname.'Response').">";
				}
			} else {
...
class wsdl extends nusoap_base {
    ...
    function wsdl($wsdl = '',$proxyhost=false,$proxyport=false,$proxyusername=false,$proxypassword=false,$timeout=0,$response_timeout=30){
        ...
        // TODO: handle imports more properly, grabbing them in-line and nesting them
			...
			//TOKR patch for document/literal element/complexType problem 
			foreach ($this->messages as $keymsg=>$message) {
			  foreach ($message as $keypar=>$parameter) {
  			  if (isset($this->messagesElementPatch[$keymsg][$keypar])) {
            $_elementName = $this->getLocalPart($parameter);
            $_elementNameNew = $_elementName.'Type';
            if ($this->currentSchema->complexTypes[$_elementName]) {
              $this->currentSchema->complexTypes[$_elementNameNew] = $this->currentSchema->complexTypes[$_elementName];
              $this->currentSchema->complexTypes[$_elementNameNew]['name'] = $_elementNameNew;
              $this->currentSchema->elements[$_elementName]['type'] .= 'Type';
              unset($this->currentSchema->complexTypes[$_elementName]);
              foreach ($this->schemas[$this->currentSchema->schemaTargetNamespace] as $kschema => $vschema) {
                if ($this->schemas[$this->currentSchema->schemaTargetNamespace][$kschema]->complexTypes[$_elementName]) {
                  $this->schemas[$this->currentSchema->schemaTargetNamespace][$kschema]->complexTypes[$_elementNameNew] = 
                    $this->schemas[$this->currentSchema->schemaTargetNamespace][$kschema]->complexTypes[$_elementName];
                  $this->schemas[$this->currentSchema->schemaTargetNamespace][$kschema]->elements[$_elementName]['type'] .= 'Type';
                  unset($this->schemas[$this->currentSchema->schemaTargetNamespace][$kschema]->complexTypes[$_elementName]);
                }
              }
            }
          }
                ...
                case 'message':
                    if ($name == 'part') {
			            ...
			            if (isset($attrs['element'])) {
			                ...
			                $this->messagesElementPatch[$this->currentMessage][$attrs['name']] = true;
			            } 
        			} 
        			break;
			    case 'portType':
    ...
    */
	//TOKR 2009-07-31 added entity_type support the same name of element and type
	function getTypeDef($type, $ns, $entity_type=false) {
	//function getTypeDef($type, $ns) {
		...
		if (isset($this->schemas[$ns])) {
			...
			for ($i = 0; $i < count($this->schemas[$ns]); $i++) {
				...
				$t = $xs->getTypeDef($type, $entity_type);
				...
				if ($t) {
					if (!isset($t['phpType'])) {
						...
						$etype = $this->getTypeDef($uqType, $ns, $entity_type);
						if ($etype) {
	...
	*/
			...
			foreach($this->bindings as $bindingName => $attrs) {
				...
				foreach($attrs['operations'] as $opName => $opParts) {
					...
					if ($opParts['input']['namespace']) {
  						...
  						$binding_xml .= '<input><soap:body use="' . $opParts['input']['use'] . '" ' . $enc_style . '/></input>';
          			}
          			//TOKR
					...
					if ($opParts['output']['namespace']) {
  						...
  						$binding_xml .= '<output><soap:body use="' . $opParts['output']['use'] . '" ' . $enc_style . '/></output>';
          			}
					//TOKR
	...
	$this->debug("in serializeType: name=$name, type=$type, use=$use, encodingStyle=$encodingStyle, unqualified=" . ($unqualified ? "unqualified" : "qualified"));
		...
		//if ($value->name) echo "NAME: ".$value->name."<Br>";
    	...
    	if (is_object($value) && get_class($value) == 'soapval') {
    		...
    		if ($value->element_ns && !preg_match('/:/', $name)) {
    			$name = $this->getPrefixFromNamespace($value->element_ns).':'.$name;
    		}
    		//TOKR end
    		if ($value->type_ns) {
		...
		if (strpos($type, ':')) {
			...
			if($ns == $this->XMLSchemaVersion || $ns == 'http://schemas.xmlsoap.org/soap/encoding/'){
				...
				//TOKR 2009-07-31 added entity_type support the same name of element and type
				if (!$this->getTypeDef($uqType, $ns, $entity_type)) {
		...
		//TOKR 2009-07-31 added entity_type support the same name of element and type
		if(!$typeDef = $this->getTypeDef($uqType, $ns, $entity_type)){
		...
		if ($phpType == 'struct') {
			if (isset($typeDef['typeClass']) && $typeDef['typeClass'] == 'element') {
				$elementName = $this->getPrefixFromNamespace($ns).':'.$uqType;
			...
			if (is_array($value)) {
				...
				if ($use == 'literal') {
					if ($forceType || $typeDef['arrayType']) {
			...
			if (is_array($value) && sizeof($value) >= 1) {
				...
				foreach($value as $k => $v) {
					...
					//TOKR - start ***********************************************
					if ($typeDef['compositor']=='sequence') {
						$elementsequence = Key($typeDef['elements']);
						$elementtype = $typeDef['elements'][$elementsequence]['type'];
						$contents .= $this->serializeType($elementsequence, $elementtype, $v, $use);
					} else
					//TOKR - end ***********************************************
					if (!in_array($typeDef['arrayType'],$this->typemap['http://www.w3.org/2001/XMLSchema'])) {
						//TOKR 2007-02-02
						//$contents .= $this->serializeType('item', $typeDef['arrayType'], $v, $use);
						$contents .= $this->serializeType($v->name?$v->name:'item', $typeDef['arrayType'], $v, $use);
					} else {
						//$contents .= $this->serialize_val($v, 'item', $typeDef['arrayType'], null, $this->XMLSchemaVersion, false, $use);
						$contents .= $this->serialize_val($v, $v->name?$v->name:'item', $typeDef['arrayType'], null, $this->XMLSchemaVersion, false, $use);
					} 
				}
			} else {
				$rows = 0;
			...
			if ($use == 'literal') {
	  			//TOKR - start ***********************************************
 				if ($typeDef['compositor']=='sequence') 
					$xml = "<$name$elementNS xsi:type=\"".$this->getPrefixFromNamespace($ns).':'.$this->getLocalPart($typeDef['name']).'">'
					.$contents
					."</$name>";
 				else 
				//TOKR - end ***********************************************
			...
			if ($use == 'literal') {
				if ($forceType) {
					$xml = "<$name$elementNS$elementAttrs xsi:type=\"" . $this->getPrefixFromNamespace($ns) . ":$uqType\">$value</$name>";
				} else {
					$xml = "<$name$elementNS$elementAttrs>$value</$name>";
				}
			} else {
				$xml = "<$name$elementNS$elementAttrs xsi:type=\"" . $this->getPrefixFromNamespace($ns) . ":$uqType\"$encodingStyle>$value</$name>";
			}
		}
		$this->debug("in serializeType: returning: $xml");
		return $xml;
...
* @access   public
*/
class TW_soapclient extends nusoap_base  {
	...
	* @access   public
	*/
	function TW_soapclient($endpoint,$wsdl = false,$proxyhost = false,$proxyport = false,$proxyusername = false, $proxypassword = false, $timeout = 0, $response_timeout = 30){
		parent::nusoap_base();
	...
	* @access   public
	*/
	function call($operation,$params=array(),$namespace='http://tempuri.org',$soapAction='',$headers=false,$rpcParams=null,$style='rpc',$use='encoded', $show_xmlns = true, $forceUse = false){
		...
		if ($style == 'rpc') {
			if ($use == 'literal') {
				...
				if ($namespace) {
					$payload = "<$operation " . ($show_xmlns ? "xmlns=\"$namespace\"" : "").">" . $payload . "</$operation>";
				} else {
				...
				if ($namespace) {
					$payload = "<$nsPrefix:$operation " . ($show_xmlns ? "xmlns:$nsPrefix=\"$namespace\"" : "").">" .
								$payload .
								"</$nsPrefix:$operation>";
				} else {
					$payload = "<$operation>" .
...
if (!class_exists('soapclient')) {
 ...
 class soapclient extends TW_soapclient  {}
